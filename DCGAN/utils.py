...
import tensorflow.contrib.slib as slim
...
def to_json(output_path, *layers):
    with open(output_path, 'w') as layer_f:
        ...
        for w, b, bn in layers:
            ...
            biases = {'sy': 1, 'depth': depth, 'w': ['{:0.2f}'.format(elem) for elem in list(B)]}

            if bn != None:
                gamma = bn.gamma.eval()
                beta = bn.beta.eval()

                gamma = {'sy': 1, 'sx': 1, 'depth': depth, 'w': ['{:0.2f}'.format(elem) for elem in list(gamma)]}
                beta = {'sy': 1, 'sx': 1, 'depth': depth, 'w': ['{0.2f}'.format(elem) for elem in list(beta)]}
            else:
            ...
            if 'lin/' in w.name:
                ...
                for w_ in W.T:
                    fs.append({'sy': 1, 'sx': 1, 'depth': W.shape[0],
                               'w': ['{:0.2f}'.format(elem) for elem in list(w_)]})
                ...
                var layer_{} = {
                  ...
                  'stride': 1, 'pad': 0,
                  'out_depth': {}, 'in_depth': {},
                  'biases': {}, 'gamma': {}, 'beta': {}, 'filters': {}
                  };'''.format(layer_idx.split('_')[0], W.shape[1], W.shape[0], biases, gamma, beta, fs)
            else:
                fs = []
                for w_ in W:
                    fs.append({'sy': 5, 'sx': 5, 'depth': W.shape[3],
                               'w': ['{:0.2f}'.format(elem) for elem in list(w_.flatten())]})
                ...
                var layer_{} = {
                  ...
                  'out_depth': {}, 'in_depth': {},
                  'biases': {}, 'gamma': {}, 'beta': {}, 'filters': {}
                  };'''.format(layer_idx, 2**(int(layer_idx)+2), 2**(int(layer_idx)+2),
